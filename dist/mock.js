(function(global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define([], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.Mock = factory());
})(this, function() {
	//#region \0rolldown/runtime.js
	var __commonJSMin = (cb, mod) => () => (mod || (cb((mod = { exports: {} }).exports, mod), cb = null), mod.exports);
	//#endregion
	//#region src/mock/constant.ts
	...
	var require_util = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		Util.each = function each(obj, iterator, context) {
			var i, key;
			if (this.type(obj) === "number") for (i = 0; i < obj; i++) iterator(i, i);
			else if (obj.length === +obj.length) {
				for (i = 0; i < obj.length; i++) if (iterator.call(context, obj[i], i, obj) === false) break;
			} else for (key in obj) if (iterator.call(context, obj[key], key, obj) === false) break;
		};
		...
		Util.Random = function(seed) {
			this.seed = seed !== void 0 ? seed : Date.now();
		};
		Util.Random.prototype = {
			constructor: Util.Random,
			next: function() {
				var t = this.seed += 1831565813;
	...
	//#endregion
	//#region src/mock/random/basic.ts
		...
		module.exports = {
			boolean: function(min, max, cur) {
				if (cur !== void 0) {
					...
					return this._random.next() > 1 / (min + max) * min ? !cur : cur;
				}
				return this._random.next() >= .5;
			},
			...
			float: function(min, max, dmin, dmax) {
				...
				for (var i = 0, dcount = this.natural(dmin, dmax); i < dcount; i++) ret += i < dcount - 1 ? this.character("number") : this.character("123456789");
			...
			string: function(pool, min, max) {
				...
				switch (arguments.length) {
					...
					case 1:
						...
						break;
					case 2:
						if (typeof arguments[0] === "string") len = min;
						else {
				...
				var text = "";
				for (var i = 0; i < len; i++) text += this.character(pool);
				return text;
			},
			str: function(...args) {
	...
	//#endregion
	//#region src/mock/random/date.ts
	var require_date = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		module.exports = {
			...
			now: function(unit, format) {
				...
				switch (unit) {
					case "week": date.setDate(date.getDate() - date.getDay());
				}
	...
	//#endregion
	//#region src/mock/random/image.ts
	var require_image = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = {
			...
			img: function(...args) {
				return this.image.apply(this, args);
			},
			_brandColors: {
				"4ormat": "#fb0a2a",
				"500px": "#02adea",
			...
			_brandNames: function() {
				var brands = [];
				for (var b in this._brandColors) brands.push(b);
				return brands;
			},
			dataImage: function(size, text) {
				var canvas;
				if (typeof document !== "undefined") canvas = document.createElement("canvas");
				else canvas = new (module.require("canvas"))();
	...
	//#endregion
	//#region src/mock/random/color_convert.ts
	var require_color_convert = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = {
			...
			hsv2hsl: function hsv2hsl(hsv) {
				var h = hsv[0], s = hsv[1] / 100, v = hsv[2] / 100, sl, l = (2 - s) * v;
				sl = s * v;
				sl /= l <= 1 ? l : 2 - l;
				l /= 2;
				return [
	...
	//#endregion
	//#region src/mock/random/color_dict.ts
	var require_color_dict = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = {
			navy: {
				value: "#000080",
				nicer: "#001F3F"
	...
	var require_color = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		module.exports = {
			...
			rgba: function() {
				var hsv = this._goldenRatioColor();
				var rgb = Convert.hsv2rgb(hsv);
				return "rgba(" + parseInt(rgb[0], 10) + ", " + parseInt(rgb[1], 10) + ", " + parseInt(rgb[2], 10) + ", " + this._random.next().toFixed(2) + ")";
			},
			hsl: function() {
				var hsv = this._goldenRatioColor();
	...
	//#endregion
	//#region src/mock/random/helper.ts
	var require_helper = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		module.exports = {
			...
			order: function order(array) {
				...
				var templatePath = orderFn.options.context.templatePath.join(".");
				var cache = orderFn.cache[templatePath] = orderFn.cache[templatePath] || {
					index: 0,
					array
				};
	...
	var require_text = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		module.exports = {
			paragraph: function(min, max) {
				...
				var result = [];
				for (var i = 0; i < len; i++) result.push(this.sentence());
				return result.join(" ");
			},
			cparagraph: function(min, max) {
				var len = range(3, 7, min, max);
				var result = [];
				for (var i = 0; i < len; i++) result.push(this.csentence());
				return result.join("");
			},
			sentence: function(min, max) {
				var len = range(12, 18, min, max);
				var result = [];
				for (var i = 0; i < len; i++) result.push(this.word());
				return Helper.capitalize(result.join(" ")) + ".";
			},
			csentence: function(min, max) {
				var len = range(12, 18, min, max);
				var result = [];
				for (var i = 0; i < len; i++) result.push(this.cword());
				return result.join("") + "。";
			},
			word: function(min, max) {
				var len = range(3, 10, min, max);
				var result = "";
				for (var i = 0; i < len; i++) result += Basic.character("lower");
				return result;
			},
			cword: function(pool, min, max) {
				...
				switch (arguments.length) {
					case 0:
						...
						break;
					case 1:
						if (typeof arguments[0] === "string") len = 1;
						else {
							len = pool;
							pool = DICT_KANZI;
						}
				...
				var result = "";
				for (var i = 0; i < len; i++) result += pool.charAt(this.natural(0, pool.length - 1));
				return result;
			},
			title: function(min, max) {
				var len = range(3, 7, min, max);
				var result = [];
				for (var i = 0; i < len; i++) result.push(this.capitalize(this.word()));
				return result.join(" ");
			},
	...
	//#endregion
	//#region src/mock/random/name.ts
	var require_name = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = {
			first: function() {
				var names = [
					"James",
	...
	//#endregion
	//#region src/mock/random/address_dict.js
	var require_address_dict = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var DICT = {
			"110000": "北京",
			"110100": "北京市",
			"110101": "东城区",
	...
	//#endregion
	//#region src/mock/random/address.ts
	var require_address = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var DICT = require_address_dict();
		var REGION = [
			"东北",
			"华北",
			"华东",
		...
		module.exports = {
			...
			county: function(prefix) {
				var province = this.pick(DICT);
				var city = this.pick(province.children);
				var county = this.pick(city.children) || { name: "-" };
				return prefix ? [
					province.name,
					city.name,
					county.name
	...
	//#endregion
	//#region src/mock/random/misc.ts
	var require_misc = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var DICT = require_address_dict();
		module.exports = {
			d4: function() {
			...
			guid: function() {
				...
				return this.string(pool, 8) + "-" + this.string(pool, 4) + "-" + this.string(pool, 4) + "-" + this.string(pool, 4) + "-" + this.string(pool, 12);
			},
			uuid: function() {
				return this.guid();
			},
			id: function() {
				...
				for (var i = 0; i < id.length; i++) sum += id[i] * rank[i];
				id += last[sum % 11];
				return id;
			},
			increment: function() {
	...
	//#endregion
	//#region src/mock/random/index.ts
	var require_random = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		var basicModule = require_basic();
		basicModule._random = Random._random;
		Random.extend(basicModule);
		Random.extend(require_date());
		Random.extend(require_image());
		Random.extend(require_color());
		Random.extend(require_text());
		Random.extend(require_name());
		Random.extend(require_web());
		Random.extend(require_address());
		Random.extend(require_helper());
		Random.extend(require_misc());
		module.exports = Random;
	}));
	//#endregion
	//#region src/mock/parser.ts
	var require_parser$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var Constant = require_constant();
		var Random = require_random();
		module.exports = { parse: function(name) {
			...
			var result = {
				...
				dmin,
				dmax,
				dcount: decimal ? !decimal[2] && parseInt(decimal[1], 10) || Random.integer(dmin, dmax) : void 0
			};
			for (var r in result) if (result[r] != void 0) return result;
			return {};
		} };
	}));
	//#endregion
	//#region src/mock/regexp/parser.ts
	var require_parser = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		module.exports = function() {
				...
				function e(n, l) {
					...
					switch (n.length) {
						...
						case 1:
							t = n[0];
							break;
						default: t = n.slice(0, -1).join(", ") + " or " + n[n.length - 1];
					}
			...
			function u(n) {
				...
				var rl, el = arguments.length > 1 ? arguments[1] : {}, ol = { regexp: c }, cl = c, il = null, al = "", fl = "|", sl = "\"|\"", hl = function(n, l) {
					...
					return new Quantifier(n, Infinity);
				}, Bl = function(n) {
	...
	//#endregion
	//#region src/mock/regexp/handler.ts
	var require_handler$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var Util = require_util();
		var Random = require_random();
		var Handler = { extend: Util.extend };
		...
		var CHARACTER_CLASSES = {
			...
			"\\S": function() {
				...
				for (var i = 0; i < SPACE.length; i++) result = result.replace(SPACE[i], "");
				return result;
			}(),
			"\\d": NUMBER,
			"\\D": LOWER + UPPER + OTHER
		...
		function ascii(from, to) {
			var result = "";
			for (var i = from; i <= to; i++) result += String.fromCharCode(i);
			return result;
		}
		...
		Handler.extend({
			token: function(node, result, cache) {
				switch (node.type) {
					case "start":
					case "end": return "";
					case "any-character": return Random.character();
					case "backspace": return "";
					case "word-boundary": return "";
					case "non-word-boundary": break;
					case "digit": return Random.pick(NUMBER.split(""));
					case "non-digit": return Random.pick((LOWER + UPPER + OTHER).split(""));
					case "form-feed": break;
					case "line-feed": return node.body || node.text;
					case "carriage-return": break;
					case "white-space": return Random.pick(SPACE.split(""));
					case "non-white-space": return Random.pick((LOWER + UPPER + NUMBER).split(""));
					case "tab": break;
					case "vertical-tab": break;
					case "word": return Random.pick((LOWER + UPPER + NUMBER).split(""));
					case "non-word": return Random.pick(OTHER.replace("_", "").split(""));
					case "null-character": break;
				}
			...
			match: function(node, result, cache) {
				result = "";
				for (var i = 0; i < node.body.length; i++) result += this.gen(node.body[i], result, cache);
				return result;
			},
			...
			quantified: function(node, result, cache) {
				...
				for (var i = 0; i < count; i++) result += this.gen(node.body, result, cache);
				return result;
			},
				...
				for (var i = 0, item; i < node.body.length; i++) {
					...
					switch (item.type) {
						...
						case "range":
							...
							for (var ii = min; ii <= max; ii++) pool = pool.replace(String.fromCharCode(ii), "");
						default:
							var characters = CHARACTER_CLASSES[item.text];
							if (characters) for (var iii = 0; iii <= characters.length; iii++) pool = pool.replace(characters[iii], "");
					}
				}
			...
			CONTROL_CHARACTER_MAP: function() {
				...
				var map = {};
				for (var i = 0; i < CONTROL_CHARACTER.length; i++) map[CONTROL_CHARACTER[i]] = CONTROL_CHARACTER_UNICODE[i];
				return map;
			}(),
	...
	//#endregion
	//#region src/mock/regexp/index.ts
	var require_regexp = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = {
			Parser: require_parser(),
			Handler: require_handler$1()
		};
	}));
	//#endregion
	//#region src/mock/handler.ts
	var require_handler = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var Constant = require_constant();
		var Util = require_util();
		var Parser = require_parser$1();
		var Random = require_random();
		var RE = require_regexp();
		var Handler = { extend: Util.extend };
		...
		Handler.extend({
			array: function(options) {
				...
				if (!options.rule.parameters) for (i = 0; i < options.template.length; i++) {
				...
				else if (options.rule.min === 1 && options.rule.max === void 0) {
					...
					options.context.path.pop();
					options.context.templatePath.pop();
				} else if (options.rule.parameters[2]) {
					...
					options.context.path.pop();
					options.context.templatePath.pop();
				} else for (i = 0; i < options.rule.count; i++) for (ii = 0; ii < options.template.length; ii++) {
			...
			object: function(options) {
				...
				if (options.rule.min != void 0) {
					...
					keys = [];
					fnKeys = [];
					for (key in options.template) (typeof options.template[key] === "function" ? fnKeys : keys).push(key);
					...
					for (i = 0; i < keys.length; i++) {
						...
						if (inc && inc[2] && Util.type(options.template[key]) === "number") options.template[key] += parseInt(inc[2], 10);
					}
				}
				return result;
			},
			number: function(options) {
				...
				if (options.rule.decimal) {
					...
					while (parts[1].length < options.rule.dcount) parts[1] += parts[1].length < options.rule.dcount - 1 ? Random.character("number") : Random.character("123456789");
					result = parseFloat(parts.join("."));
				} else result = options.rule.range && !options.rule.parameters[2] ? options.rule.count : options.template;
				return result;
			},
			boolean: function(options) {
				return options.rule.parameters ? Random.bool(options.rule.min, options.rule.max, options.template) : options.template;
			},
			string: function(options) {
				var result = "", i, placeholders, ph, phed;
				if (options.template.length) {
					if (options.rule.count == void 0) result += options.template;
				...
				} else result = options.rule.range ? Random.string(options.rule.count) : options.template;
				return result;
			},
			...
			"regexp": function(options) {
				var source = "";
				if (options.rule.count == void 0) source += options.template.source;
				for (var i = 0; i < options.rule.count; i++) source += options.template.source;
		...
		Handler.extend({
			...
			placeholder: function(placeholder, obj, templateContext, options) {
				...
				if (key.charAt(0) === "/" || pathParts.length > 1) return this.getValueByPath(key, options);
				...
				for (var i = 0; i < params.length; i++) {
					Constant.RE_PLACEHOLDER.exec("");
					if (Constant.RE_PLACEHOLDER.test(params[i])) params[i] = Handler.placeholder(params[i], obj, templateContext, options);
				}
				var handle = Random[key] || Random[lkey] || Random[okey];
				switch (Util.type(handle)) {
					case "array": return Random.pick(handle);
					case "function":
			...
			getValueByPath: function(key, options) {
				var originalKey = key;
				var keyPathParts = this.splitPathToArray(key);
				var absolutePathParts = [];
				if (key.charAt(0) === "/") absolutePathParts = [options.context.path[0]].concat(this.normalizePath(keyPathParts));
				else if (keyPathParts.length > 1) {
			...
			normalizePath: function(pathParts) {
				var newPathParts = [];
				for (var i = 0; i < pathParts.length; i++) switch (pathParts[i]) {
					case "..":
						newPathParts.pop();
						break;
					case ".": break;
					default: newPathParts.push(pathParts[i]);
				}
				return newPathParts;
			},
	...
	//#endregion
	//#region src/mock/schema/schema.ts
	...
	//#endregion
	//#region src/mock/schema/index.ts
	var require_schema = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = require_schema$1();
	}));
	//#endregion
	//#region src/mock/valid/valid.ts
	var require_valid$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var Constant = require_constant();
		var Util = require_util();
		var toJSONSchema = require_schema();
		...
		var Diff = {
			...
			value: function(schema, data, name, result) {
				...
				switch (templateType) {
					case "number":
						...
						if (rule.min !== void 0 && rule.max === void 0) Assert.equal("value", schema.path, parts[0], rule.min, result, "[value] " + name);
						if (rule.decimal) {
							if (rule.dmin !== void 0 && rule.dmax !== void 0) {
								Assert.greaterThanOrEqualTo("value", schema.path, parts[1].length, rule.dmin, result);
								Assert.lessThanOrEqualTo("value", schema.path, parts[1].length, rule.dmax, result);
							}
							if (rule.dmin !== void 0 && rule.dmax === void 0) Assert.equal("value", schema.path, parts[1].length, rule.dmin, result);
						}
						break;
					case "boolean": break;
					case "string":
						...
						if (rule.min !== void 0 && rule.max === void 0) Assert.equal("repeat count", schema.path, actualRepeatCount, rule.min, result);
						break;
					case "regexp":
			...
			properties: function(schema, data, name, result) {
				...
				if (!schema.rule.parameters) Assert.equal("properties length", schema.path, keys.length, schema.properties.length, result);
				else {
					if (rule.min !== void 0 && rule.max !== void 0) {
				...
				for (var i = 0; i < keys.length; i++) result.push.apply(result, this.diff((function() {
					var property;
			...
			items: function(schema, data, name, result) {
				...
				var rule = schema.rule;
				if (!schema.rule.parameters) Assert.equal("items length", schema.path, data.length, schema.items.length, result);
				else {
					if (rule.min !== void 0 && rule.max !== void 0) {
					...
					if (rule.min !== void 0 && rule.max === void 0) if (rule.count === 1) return result.length === length;
					else Assert.equal("items length", schema.path, data.length, rule.min * schema.items.length, result);
					if (rule.parameters[2]) return result.length === length;
				}
				if (result.length !== length) return false;
				for (var i = 0; i < data.length; i++) result.push.apply(result, this.diff(schema.items[i % schema.items.length], data[i], i % schema.items.length));
				return result.length === length;
			}
		};
		var Assert = {
	...
	//#endregion
	//#region src/mock/valid/index.ts
	var require_valid = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = require_valid$1();
	}));
	//#endregion
	//#region src/mock/xhr/xhr.ts
	var require_xhr$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		var Util = require_util();
		...
		MockXMLHttpRequest._settings = { timeout: "10-100" };
		...
		Util.extend(MockXMLHttpRequest.prototype, {
			open: function(method, url, async, username, password) {
				...
				function handle(event) {
					for (var i = 0; i < XHR_RESPONSE_PROPERTIES.length; i++) try {
				...
				if (!item) {
					...
					this.custom.xhr = xhr;
					for (var i = 0; i < XHR_EVENTS.length; i++) xhr.addEventListener(XHR_EVENTS[i], handle);
					if (username) xhr.open(method, url, async, username, password);
					else xhr.open(method, url, async);
					for (var j = 0; j < XHR_REQUEST_PROPERTIES.length; j++) try {
		...
		Util.extend(MockXMLHttpRequest.prototype, {
			...
			getResponseHeader: function(name) {
				if (!this.match) return this.custom.xhr.getResponseHeader(name);
		...
		Util.extend(MockXMLHttpRequest.prototype, {
			...
			removeEventListener: function removeEventListener(type, handle) {
				...
				for (var i = 0; i < handles.length; i++) if (handles[i] === handle) handles.splice(i--, 1);
			},
			dispatchEvent: function dispatchEvent(event) {
				var handles = this.custom.events[event.type] || [];
				for (var i = 0; i < handles.length; i++) handles[i].call(this, event);
		...
		function find(options) {
			for (var sUrlType in MockXMLHttpRequest.Mock._mocked) {
				...
				if ((!item.rurl || match(item.rurl, options.url)) && (!item.rtype || match(item.rtype, options.type.toLowerCase()))) return item;
			}
			function match(expected, actual) {
				if (Util.type(expected) === "string") return expected === actual;
				if (Util.type(expected) === "regexp") return expected.test(actual);
				return false;
			}
		}
	...
	//#endregion
	//#region src/mock/xhr/index.ts
	var require_xhr = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		module.exports = require_xhr$1();
	}));
	//#endregion
	//#region src/mock.ts
	var require_mock = /* @__PURE__ */ __commonJSMin(((exports, module) => {
		...
		var XHR;
		if (typeof window !== "undefined") XHR = require_xhr();
		var Mock = {
			Handler,
			Random,
			Util,
