...
describe('DPD', () => {
    ...
    describe('Priority', () => {
        it('@EMAIL with override', () => {
            var data = Mock.mock({
                email: 'nuysoft@gmail.com',
                name: '@EMAIL'
            })
            expect(data.name).not.toEqual(data.email)
        })
        it('@email case-sensitive', () => {
            ...
            expect(data.name).toEqual(data.email)
        })
    })
    describe('Escape', () => {
        // Mock.mock('@EMAIL') returns a random email, not the literal
        it('@EMAIL', () => {
            var data = Mock.mock('@EMAIL')
            expect(data).not.toEqual('@EMAIL')
        })
        // Mock.mock('\@EMAIL') — \@ is an escaped placeholder, result differs from input
        it('backslash @EMAIL', () => {
            var data = Mock.mock('\\@EMAIL')
            expect(data).toBeDefined()
        })
        it('double backslash @EMAIL', () => {
            var data = Mock.mock('\\\@EMAIL')
            expect(data).toBeDefined()
        })
        it('triple backslash @EMAIL', () => {
            var data = Mock.mock('\\\\@EMAIL')
            expect(data).toBeDefined()
        })
    })
    describe('Path', () => {
        it('Absolute Path', () => {
            ...
            expect(data.children[0]).toHaveProperty('parentId', data.id)
            expect(data.child).toHaveProperty('parentId', data.id)
        })
        it('Relative Path', () => {
            var data = Mock.mock({
                id: '@UUID',
        ...
        it('Relative Path Fallback in Corner Case', () => {
            const STRING = 'https://test.org/@a/build/test.html'
            var data = Mock.mock({ a: STRING })
            expect(data.a).toEqual(STRING)
        })
    })
    describe('Complex', () => {
        var tpl = {
